---
type: lecture
date: 2026-02-11T18:01:00+4:30
title: Agent Tool Design
tldr: "An Introduction to Model Context Protocol."
thumbnail: /static_files/presentations/lec.jpg
links:
    - url: /static_files/07_agent-tool-design.pdf
      name: slides
---
**Suggested Readings:**
- [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro)
- [MCP Tools Beyond API](https://www.reillywood.com/blog/apis-dont-make-good-mcp-tools/)
- [MCP Registry](https://blog.modelcontextprotocol.io/posts/2025-09-08-mcp-registry-preview/)
- [Build a Remote MCP Server](https://developers.cloudflare.com/agents/guides/remote-mcp-server/)
